...
test('should display the location list when user input something', async () => {
  ...
  fetch.mock(`${locationUri}/search/?query=${typingValue}`, { status: 200, body: { data: locations }});

  const { getByText, getByPlaceholderText } = render(<LocationSearchBar onSearch={jest.fn()}/>);
  fireEvent.change(getByPlaceholderText(/Type location/i), { target: { value: typingValue } });
